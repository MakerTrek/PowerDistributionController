...
LOG_MODULE_REGISTER(vesc, LOG_LEVEL_INF);

const struct device * _can_dev = NULL;

int vesc_init(const struct device * can_dev) {
    _can_dev = can_dev;

    if (!device_is_ready(_can_dev)) {
		printf("CAN: Device %s not ready.\n", _can_dev->name);
		return 0;
	}

	int ret = can_start(_can_dev);
	if (ret != 0) {
...
void can_transmit_eid(uint32_t id, const uint8_t *data, uint8_t len) {
    ...
    can_mode_t saved_can_mode = can_get_mode(_can_dev);

    can_set_mode(_can_dev, CAN_MODE_NORMAL | CAN_MODE_ONE_SHOT);
    ...
    int ret = can_send(_can_dev, &frame, K_NO_WAIT, NULL, NULL);
    ...
    can_set_mode(_can_dev, saved_can_mode);
}
